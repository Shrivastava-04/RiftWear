...
const EditProductModal = ({ isOpen, onClose, product, onProductUpdated }) => {
  ...
  useEffect(() => {
    if (product) {
      const productCopy = JSON.parse(JSON.stringify(product));
      const variantsWithUrlState = productCopy.variants.map((variant) => ({
        ...variant,
        sizeChart: Array.isArray(variant.sizeChart) ? variant.sizeChart : [],
        colors: variant.colors.map((color) => ({
          ...color,
          newImageUrls: [""],
        })),
      }));
      setFormData({ ...productCopy, variants: variantsWithUrlState });
      setNewMainImageUrls([""]);
    }
  }, [product]);
  ...
  const handleColorUrlChange = (vIndex, cIndex, urlIndex, value) => {
    setFormData((prev) => {
      ...
      if (!color.newImageUrls) color.newImageUrls = [""];
      color.newImageUrls[urlIndex] = value;
      return { ...prev, variants: newVariants };
    });
  };
  ...
  const removeColorUrlInput = (vIndex, cIndex, urlIndex) => {
    setFormData((prev) => {
      const newVariants = JSON.parse(JSON.stringify(prev.variants));
      const color = newVariants[vIndex].colors[cIndex];
      color.newImageUrls = color.newImageUrls.filter((_, i) => i !== urlIndex);
      return { ...prev, variants: newVariants };
    });
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const finalVariantsData = formData.variants.map((variant) => {
        const finalColors = variant.colors.map((color) => ({
          _id: color._id,
          name: color.name,
          images: [
            ...color.images,
      ...
      <DialogContent className="sm:max-w-[800px] max-h-[90vh] overflow-y-auto">
        ...
        <div className="py-4 space-y-8">
          ...
          <div className="space-y-4 p-4 border rounded-lg">
            ...
            <div className="space-y-2">
              <Label>Add New Image URLs</Label>
              {newMainImageUrls.map((url, index) => (
