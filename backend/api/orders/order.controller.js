...
// // Add this new function to your order.controller.js
// const orderStatusUpdateHtmlContent = (orderNumber, userName, batchNumber) => {
//   const userFirstName = userName.split(" ")[0]; // Get the user's first name
...
//       // Find the specific variant from the product data sent in the payload
//       const variant = item.productId.variants.find(
//         (v) => v._id === item.variantId
//       );
//       if (!variant) {
//         // This is a safety check in case the data is inconsistent
//         throw new Error(
//           `Variant with ID ${item.variantId} not found in product ${item.productId.name}`
//         );
//       }
...
//     // --- Create the new order with the robust schema ---
...
//     // (The rest of the function for clearing cart and sending email remains the same)
...
// // Replace the existing function with this final version
// // Replace the existing function with this final version
    ...
    if (dbUser) {
      ...
      try {
        const emailHtml = orderConfirmedHtmlContent(
          ...
          dbUser.name,
          itemsSnapshot, // Use the snapshot we already created
