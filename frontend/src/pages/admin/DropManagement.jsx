...
// --- UPDATED: Helper to format UTC date string into IST date/time for form inputs ---
...
const DropFormModal = ({ isOpen, onClose, drop, onSave }) => {
  ...
  useEffect(() => {
    if (drop) {
      ...
      setFormData({
        ...
        description: "",
        startDate: "",
        startTime: "18:00", // Default to 6:00 PM IST
        endDate: "",
        endTime: "23:59", // Default to 11:59 PM IST
        isActive: false,
      });
    }
  }, [drop, isOpen]);
  ...
  const handleSubmit = async (e) => {
    ...
    // --- UPDATED: Combine date and time, explicitly setting IST timezone offset ---
    // This ensures the backend receives the correct moment in time as UTC
    const finalData = {
      ...formData,
      startDate: new Date(
        `${formData.startDate}T${formData.startTime}:00+05:30`
      ),
      endDate: new Date(`${formData.endDate}T${formData.endTime}:00+05:30`),
    };
      ...
      <DialogContent>
        ...
        <form onSubmit={handleSubmit} className="py-4 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <Label>Start Time (IST 24hr)</Label>
              <Input
                type="time"
                name="startTime"
                value={formData.startTime || ""}
          ...
          <div className="flex items-start space-x-2 pt-2">
            ...
            />
            <div>
              <Label htmlFor="isActive">Activate Drop</Label>
              <p className="text-xs text-muted-foreground">
                Note: Activating this will deactivate any other active drop.
              </p>
            </div>
          </div>
          <DialogFooter>
...
const DropManagement = () => {
  ...
  // --- UPDATED: Formats UTC date to IST (24-hour) for display ---
  const formatDateTime = (dateString) =>
    new Date(dateString).toLocaleString("en-IN", {
      timeZone: "Asia/Kolkata",
      dateStyle: "medium",
  ...
  // --- NEW: Determines the live status of a drop ---
  const getLiveStatus = (startDate, endDate) => {
    const now = new Date();
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          <div>
            ...
            <CardDescription>
              Create and manage timed release events for products.
            </CardDescription>
          </div>
          <Button onClick={() => openModal()}>
            <Plus className="h-4 w-4 mr-2" />
                ...
                <TableBody>
                  {drops.map((drop) => (
                    <TableRow key={drop._id}>
                      ...
                      <TableCell>{formatDateTime(drop.endDate)}</TableCell>
                      <TableCell>
                        {getLiveStatus(drop.startDate, drop.endDate)}
                      </TableCell>
                      <TableCell>
                        <Badge
                          variant={drop.isActive ? "success" : "secondary"}
