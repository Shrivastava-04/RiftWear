...
import {
  adminGetAllOrders,
  adminUpdateOrderStatus,
  exportOrders, // Import the export function
} from "../../api/apiService";
import OrderDetailsModal from "../../components/admin/OrderDetailsModal";
...
const OrderManagement = () => {
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [statusFilter, sortBy, dateFilter]);
  ...
  const handleSelectAllOnPage = (checked) => {
    const pageOrderIds = currentOrders.map((o) => o._id);
    if (checked) {
  ...
  const handleExport = async () => {
    if (!dateFilter.from || !dateFilter.to) {
      toast({
        ...
        description: "Please select a start and end date to export orders.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      toast({
        title: "Exporting...",
        description: "Preparing your orders file.",
      });
      const response = await exportOrders(dateFilter.from, dateFilter.to);
      ...
      link.setAttribute(
        "download",
        `orders_${dateFilter.from}_to_${dateFilter.to}.xlsx`
      );
      ...
      toast({
        title: "Export Failed",
        description:
          err.response?.data?.message || "Could not generate the export file.",
        variant: "destructive",
      });
    } finally {
            ...
            <>
              ...
              <div className="flex items-center justify-between pt-4 mt-4 border-t border-border/50">
                <div className="text-sm text-muted-foreground">
                  Selected {selectedOrders.length} of{" "}
                  {filteredAndSortedOrders.length} order(s).
                </div>
